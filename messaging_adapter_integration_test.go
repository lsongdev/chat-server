...
func TestDeliveryCoreWithChatPostgresStore(t *testing.T) {
	...
	engine, err := messaging.New(messaging.Options{
		Authenticate: func(_ context.Context, request *http.Request) (messaging.Identity, error) {
			return messaging.Identity{ID: request.Header.Get("X-Test-Identity")}, nil
		},
...
func dialChatDelivery(t *testing.T, serverURL string, identityID uuid.UUID) *websocket.Conn {
	...
	dialer.Subprotocols = []string{messaging.Subprotocol}
